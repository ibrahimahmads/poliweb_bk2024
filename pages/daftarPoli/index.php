                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-nowrap">
                        ...
                        <tbody>
                            ...
                            require 'config/koneksi.php';
                            $no = 1;
                            $query = "SELECT daftar_poli.id as idDaftarPoli, poli.nama_poli, dokter.nama, jadwal_periksa.hari, jadwal_periksa.jam_mulai, jadwal_periksa.jam_selesai, 
                            daftar_poli.no_antrian, daftar_poli.status_periksa FROM daftar_poli INNER JOIN jadwal_periksa ON daftar_poli.id_jadwal = jadwal_periksa.id INNER JOIN dokter 
                            ON jadwal_periksa.id_dokter = dokter.id INNER JOIN poli ON dokter.id_poli = poli.id WHERE daftar_poli.id_pasien = '$idPasien'";
                            $result = mysqli_query($mysqli, $query);
                            ...
                            <tr class="text-center">
                                ...
                                <td>
                                    <?php if ($data['status_periksa'] == 0) { ?>
                                        ...
                                        class='btn btn-sm edit-btn text-white' style="background-color: #0284c7;">Detail</a>
                                    <?php } elseif ($data['status_periksa'] == 1) { ?>
                                        <a href="detailDaftarPoli.php?id=<?php echo $data['idDaftarPoli'] ?>"
                                        class='btn btn-sm btn-success edit-btn'>Riwayat</a>
                                    <?php } ?>
